...
declare type KeyPathToNestedObject<S extends string> = S extends `${infer T}.${infer U}` ? {
    [Key in T]: KeyPathToNestedObject<U>;
} : S;
declare type OptionalNested<Type, PathToKey> = PathToKey extends object ? Omit<Type, keyof PathToKey> & {
...
declare type ValueWithOptionalKeyPath<Value extends unknown, KeyPath extends string> = OptionalNested<Value, KeyPathToNestedObject<KeyPath>>;
export declare type StoreValueWithOptionalKey<DBTypes extends DBSchema | unknown, StoreName extends StoreNames<DBTypes>> = DBTypes extends DBSchema ? DBTypes[StoreName]['autoIncrementKeyPath'] extends string ? (ValueWithOptionalKeyPath<DBTypes[StoreName]['value'], DBTypes[StoreName]['autoIncrementKeyPath']>) : StoreValue<DBTypes, StoreName> : any;
/**
...
export interface IDBPDatabase<DBTypes extends DBSchema | unknown = unknown> extends IDBPDatabaseExtends {
     ...
     */
    createObjectStore<Name extends StoreNames<DBTypes>>(name: Name, optionalParameters?: DBTypes extends DBSchema ? DBTypes[Name]['autoIncrementKeyPath'] extends string ? (IDBObjectStoreParameters & {
        autoIncrement: true;
        keyPath: DBTypes[Name]['autoIncrementKeyPath'];
    }) : IDBObjectStoreParameters : IDBObjectStoreParameters): IDBPObjectStore<DBTypes, ArrayLike<StoreNames<DBTypes>>, Name, 'versionchange'>;
    /**
     ...
     * @param key
     */
    add<Name extends StoreNames<DBTypes>>(storeName: Name, value: StoreValueWithOptionalKey<DBTypes, Name>, key?: StoreKey<DBTypes, Name> | IDBKeyRange): Promise<StoreKey<DBTypes, Name>>;
    /**
...
export interface IDBPObjectStore<DBTypes extends DBSchema | unknown = unknown, TxStores extends ArrayLike<StoreNames<DBTypes>> = ArrayLike<StoreNames<DBTypes>>, StoreName extends StoreNames<DBTypes> = StoreNames<DBTypes>, Mode extends IDBTransactionMode = 'readonly'> extends IDBPObjectStoreExtends {
     ...
     * Rejects if an item of a given key already exists in the store.
     */
    add: Mode extends 'readonly' ? undefined : (value: StoreValueWithOptionalKey<DBTypes, StoreName>, key?: StoreKey<DBTypes, StoreName> | IDBKeyRange) => Promise<StoreKey<DBTypes, StoreName>>;
    /**
     ...
     * Replaces any item with the same key.
     */
    put: Mode extends 'readonly' ? undefined : (value: StoreValueWithOptionalKey<DBTypes, StoreName>, key?: StoreKey<DBTypes, StoreName> | IDBKeyRange) => Promise<StoreKey<DBTypes, StoreName>>;
    /**
